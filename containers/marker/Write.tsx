...
import Images from "@/components/Doc/Images";
...
type EditorProps = {
    // Define the props of your Editor component here
};
...
const Write: React.FC = () => {
    ...
    const handleImageUpload = async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        try {
            ...
            setState((prevState: States) => ({
                ...prevState,
                url: [...prevState.url, ...urls],
            }));
    ...
    const onSubmit = async () => {
        try {
            ...
            await setData('marker', lastModAt.toString(), state).then((res) => {
    ...
    const onChange = (e: any) => {
        e.target.value ? setState({ ...state, [e.target.name]: e.target.value }) : setState({ ...state, [e.target.name]: e.target.files[0] });
    };
    ...
    const { title, content, name, address, url, coord } = state;
    return (
        <>
            <Script
            ...
            < div className="form" >
                ...
                <div className="input-group">
                    ...
                    />
                    <button onClick={(e) => handleImageUpload(e)} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded me-3">업로드</button>
                    <Preview />
                </div>
                <div className="input-group">
                    <label htmlFor="coord">좌표</label>
                    ...
                    <input type="button" className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded me-3" value="좌표 검색" onClick={() => getCoords(address)} />
                </div>
            </div >
